...
Animation time! To animate the travels, we needed some kind of timer and variables to keep track of the animation's state. Processing's `millis` would be our de-facto timer: by keeping track of `animationLastTick` and defining an `animationPathTime`, we can set a timer to increase each journey's entry in `journeyIndices` every, say, 1 second and thus move from one path segment to the next. The idea is that every "tick", we draw the path from one country to the next, and we keep going until reaching the end of the journey. Thansk to our normalized `lon` and `lat` fields from our client submissions, intermolating and drawing our journeys in the map becomes super simple! Just take the normalized `lon` and `lat`, multiply by the map's `width` and `height` and done! (All hail normalization).
